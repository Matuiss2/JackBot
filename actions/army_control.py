...
class ArmyControl(micro):
    ...
    def move_to_rallying_point(self, unit):
        ...
        self.ai.actions.append(
            unit.move(
                self.ai.townhalls.closest_to(self.ai._game_info.map_center).position.towards(self.ai._game_info.map_center, 10)
            )
        )
    ...
    def retreat_unit(self, unit, filtered_enemies):
        ...
        if (
            self.ai.townhalls
            and not self.ai.units.structure.closer_than(15, unit.position)
            and len(filtered_enemies.exclude_type({DRONE, SCV, PROBE}).closer_than(15, unit.position))
            >= len(self.ai.zerglings.closer_than(15, unit.position))
            + len(self.ai.ultralisks.closer_than(15, unit.position)) * 4
        ):
