...
class Overseer:
    ...
    async def handle(self, iteration):
        ...
        if atk_force:
            self.ai.adding(selected_ov.move(atk_force.closest_to(selected_ov.position)))
        elif self.ai.townhalls:
            self.ai.adding(selected_ov.move(self.ai.townhalls.closest_to(selected_ov.position)))
