...
def combine_actions(action_iter):
    ...
    for key, items in groupby(action_iter, key=lambda a: a.combining_tuple):
        ...
        if combineable:
            ...
            # Return one action for each unit; this is required for certain commands that would otherwise be grouped,
            # and only executed once
            # Examples:
            # Select 3 hatcheries, build a queen with each hatch - the grouping function would group these unit tags
            # and only issue one train command once to all 3 unit tags - resulting in one total train command
            # I imagine the same thing would happen to certain other abilities: Battlecruiser yamato on same target,
            # queen transfuse on same time, ghost snipe on same target, all build commands with the same unit type
            # and also all morphs (zergling to banelings)
            # However, other abilities can and should be grouped, see constants.py 'COMBINEABLE_ABILITIES'
            if target is None:
                for unit in items:
                    cmd = raw_pb.ActionRawUnitCommand(
                        ability_id=ability.value, unit_tags={unit.unit.tag}, queue_command=queue
                    )
                    yield raw_pb.ActionRaw(unit_command=cmd)
            elif isinstance(target, Point2):
                for unit in items:
