...
from .data import ACTION_RESULT, ALERT, RACE, RESULT, TARGET, race_gas, race_townhalls, race_worker
...
class BotAI(DistanceCalculation):
    ...
    def _initialize_variables(self):
        ...
        if not hasattr(self, "opponent_id"):
            ...
            # -> https://github.com/Hannessa/python-sc2-ladderbot/blob/master/__init__.py#L40
        ...
        self.destructibles: Units = Units([], self)
    ...
    def time_formatted(self) -> str:
        ...
        time_unformatted = self.time
        return f"{int(time_unformatted // 60):02}:{int(time_unformatted % 60):02}"
    ...
    def alert(self, alert_code: ALERT) -> bool:
        ...
        Possible alerts are listed here:
        https://github.com/Blizzard/s2client-proto/blob/e38efed74c03bec90f74b330ea1adda9215e655f/s2clientprotocol/sc2api.proto#L679-L702
    ...
    def main_base_ramp(self) -> Ramp:
        ...
        try:
            ...
            # Hardcoded hot-fix for Honorgrounds LE map, as that map has a large main base ramp with in-base natural
            self.cached_main_base_ramp = min(
    ...
    async def get_available_abilities(
        ...
        """ Returns available abilities of one or more units. Right now only checks cool-down, energy cost
    ...
    async def get_next_expansion(self) -> Optional[Point2]:
        ...
        for expansion_location in self.expansion_locations:
            ...
            distance = await self._client.query_pathing(startp, expansion_location)
            if distance is None:
                continue
    ...
    def calculate_supply_cost(self, unit_type: UnitTypeId) -> float:
        ...
        The required supply to build zerglings is 1 because they pop in pairs, so this function returns 1 because
        the larva morph command requires 1 free supply.
    ...
    def calculate_cost(self, item_id: Union[UnitTypeId, UpgradeId, AbilityId]) -> Cost:
        """
        Calculate the required build, train or morph cost of a unit. It is recommended to use the UnitTypeId instead of
        the ability to create the unit.
        The total cost to create a ravager is 100/100, but the actual morph cost from roach to ravager is only 25/75,
        so this function returns 25/75.
    ...
    def can_afford(self, item_id: Union[UnitTypeId, UpgradeId, AbilityId], check_supply_cost: bool = True) -> bool:
        ...
        Example::
            ...
            # Will be 'True' although the API reports that an orbital is worth 550 minerals, but the morph cost is only
            150 minerals
    ...
    async def can_cast(
        ...
        Example::
            ...
            stalkers_that_can_blink = stalkers.filter(lambda unit: unit.type_id == UnitTypeId.STALKER and
    ...
    async def can_place(self, building: Union[AbilityData, AbilityId, UnitTypeId], position: Point2) -> bool:
        ...
        results = await self._client.query_building_placement(building, [position])
        return results[0] == ACTION_RESULT.Success
    ...
    def already_pending_upgrade(self, upgrade_type: UpgradeId) -> Union[int, float]:
        ...
        for structure in self.structures.filter(lambda unit: unit.is_ready):
            for order in structure.orders:
                if order.ability.id is creation_ability_id:
    ...
    async def build(
        ...
        """ Not recommended as this function checks many positions
        if it "can place" on them until it found a valid position.
        Also if the given position is not placeable,
        this function tries to find a nearby position to place the structure.
        ...
        builder = build_worker or self.select_build_worker(placement)
        if builder is None:
            return False
    ...
    def train(
        ...
        Example Zerg::
            ...
            # This should queue 5 queens in 5 different townhalls if you have enough townhalls, enough minerals and
             enough free supply left
        ...
        for structure in train_structures:
            ...
            if (
                ...
                # Either parameter "train_only_idle_buildings" is False or structure is idle
                # or structure has less than 2 orders and has reactor
                and (
            ...
            for tech_alias in list(structure_data._proto.tech_alias) + [structure._proto.unit_type]:
                if tech_alias == structure_type_value:
                    if structure.build_progress == 1:
                        return 1
                    if return_value < structure.build_progress:
    ...
    def tech_requirement_progress(self, structure_type: UnitTypeId) -> float:
        ...
        Example::
            ...
            print(tech_requirement) # Prints 1 because even though the type id of the flying factory is different,
            it still has build progress of 1 and thus tech requirement is completed
    ...
    def _prepare_units(self):
        ...
        for unit in self.state.observation_raw.units:
            if unit.is_blip:
                ...
                # Convert these units to effects: reaper grenade, parasitic bomb dummy, force-field
                ...
                if alliance == 3:
                    ...
                    # all destructible rocks
                        ...
                        self.destructibles.append(unit_obj)
    ...
    async def _issue_building_events(self):
        for structure in self.structures:
            ...
            if structure.build_progress < 1:
                continue
            # Using get function in case somehow the previous structure map (last frame) does not contain this structure
