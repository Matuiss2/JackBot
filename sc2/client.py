...
class Client(Protocol):
    def __init__(self, ws):
        ...
        # Store hash value of the debug requests to prevent sending the same 1 again if they haven't changed last frame
    ...
    async def join_game(self, name=None, race=None, observed_player_id=None, portconfig=None, rgb_render_config=None):
        ...
        if portconfig:
            ...
            for ppc in portconfig.players:
                request = req.client_ports.add()
                request.game_port = ppc[0]
    ...
    async def step(self):
        """ EXPERIMENTAL: Change self._client.game_step during the step function to increase or decrease steps/s """
    ...
    async def chat_send(self, message: str, team_only: bool):
        ...
        channel = CHAT_CHANNEL.Team if team_only else CHAT_CHANNEL.Broadcast
        await self._execute(
            action=sc_pb.RequestAction(
                actions=[sc_pb.Action(action_chat=sc_pb.ActionChat(channel=channel.value, message=message))]
            )
        )
    ...
    def debug_box2_out(
        ...
        """ Draws a box center at a position 'pos', with box side lengths (vertices) of 2 times 'half_vertex_length'."""
        if isinstance(pos, Unit):
            pos = pos.position3d
        elif not isinstance(pos, Point3):
            pos = Point3((pos.x, pos.y, 0))
        po0 = pos + Point3((-half_vertex_length, -half_vertex_length, -half_vertex_length))
        po1 = pos + Point3((half_vertex_length, half_vertex_length, half_vertex_length))
        self._debug_boxes.append(DrawItemBox(start_point=po0, end_point=po1, color=color))

    def debug_sphere_out(
        self, point: Union[Unit, Point2, Point3], radius: Union[int, float], color: Union[tuple, list, Point3] = None
    ):
        """ Draws a sphere at point p with radius r. """
        self._debug_boxes.append(DrawItemSphere(start_point=point, radius=radius, color=color))

    async def _send_debug(self):
        """ Sends the debug draw execution. This is run by main.py now automatically, if there is any items in the list.
        You do not need to run this manually any longer.
        Check examples/terran/ramp_wall.py for example drawing.
        Each draw request needs to be sent again in every single on_step iteration.
        """
        debug_hash = (
    ...
    async def debug_set_unit_value(self, unit_tags: Union[Iterable[int], Units, Unit], unit_value: int, value: float):
        ...
        Can't set the life of a unit to 0, use "debug_kill_unit" for that.
        Also can't set the life above the unit's maximum.
    ...
    async def debug_control_enemy(self):
        """ Allows control over enemy units and structures similar to team games control
         does not allow the bot to spend the opponent's resources using it a second time disables it again.  """
    ...
    async def debug_free(self):
        """ Units, structures and upgrades are free of mineral and gas cost. Using it a 2nd time disables it again.  """
    ...
    async def debug_upgrade(self):
        """ Researches all currently available upgrades.
        E.g. using it once unlocks combat shield, stimpack and 1-1.
    ...
    async def debug_fast_build(self):
        """ Sets the build time of units and structures and upgrades to 0. Using it a second time disables it again. """
...
class DrawItem:
    ...
    def to_debug_color(color: Union[tuple, Point3]):
        ...
        if isinstance(color, (tuple, list)) and not isinstance(color, Point3) and len(color) == 3:
            return debug_pb.Color(r=color[0], g=color[1], b=color[2])  # In case color is of type Point3
