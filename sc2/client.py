...
class Client(Protocol):
    ...
    async def join_game(self, race=None, observed_player_id=None, portconfig=None):
        ...
        if portconfig:
            ...
            for ppc in portconfig.players:
                port = req.client_ports.add()
                port.game_port = ppc[0]
                port.base_port = ppc[1]
    ...
    async def save_replay(self, path):
        """Save the replay on the specified path"""
    ...
    async def step(self):
        """ Change self._client.game_step during the step function to increase or decrease steps per second """
        result = await self._execute(step=sc_pb.RequestStep(count=self.game_step))
        return result

    async def get_game_data(self) -> GameData:
        """Gets played game data"""
    ...
    async def actions(self, actions, game_data, return_successes=False):
        """Returns a list of successful actions, or if there is None return the unsuccessful ones"""
    ...
    async def debug_create_unit(self, unit_spawn_commands: List[List[Union[UnitTypeId, int, Point2, Point3]]]):
        ...
        assert unit_spawn_commands[0][1]  # careful, in realtime=True this function may create more units
    ...
    async def debug_kill_unit(self, unit_tags: Union[Units, List[int], Set[int]]):
        """Is triggered when an unit is killed on the place it got killed"""
    ...
    def debug_text_simple(self, text: str):
        """ Draws a text in the top left corner of the screen (up to a max of 6 messages it seems).
         Don't forget to add 'await self._client.send_debug'. """
    ...
    def debug_line_out(self, point0: Union[Unit, Point2, Point3], point1: Union[Unit, Point2, Point3], color=None):
        """ Draws a line from p0 to p1. Don't forget to add 'await self._client.send_debug'. """
        self._debug_lines.append(
            debug_pb.DebugLine(
                line=debug_pb.Line(p0=self.to_debug_point(point0), p1=self.to_debug_point(point1)),
    ...
    def debug_sphere_out(self, point: Union[Unit, Point2, Point3], radius: Union[int, float], color=None):
        """ Draws a sphere at point p with radius r. Don't forget to add 'await self._client.send_debug'. """
        self._debug_spheres.append(
            debug_pb.DebugSphere(p=self.to_debug_point(point), r=radius, color=self.to_debug_color(color))
        )
