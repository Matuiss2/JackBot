...
class DistanceCalculation:
    def __init__(self):
        ...
        self._distance_squared_unit_to_unit = self.calculate_distances = self._get_index_of_two_units = None
    ...
    def _unit_index_dict(self) -> Dict[int, int]:
        """ As property, so it will be recalculated each time it is called,
         or return from cache if it is called multiple times in teh same game_loop. """
    ...
    def _calculate_distances_method1(self) -> np.ndarray:
        if self._generated_frame2 != self.state.game_loop:
            ...
            #             error_margin = 1e-5
            #             assert (abs(pdist_distance - correct_dist) < error_margin),
            #             f"Actual distance is {correct_dist} but calculated pdist distance is {pdist_distance}"
            #         except:
            #             print(
            #                 f"Error caused by unit1 {unit1} and unit2 {unit2} with positions {unit1.position_tuple}
            #                 and {unit2.position_tuple}"
            #             )
            #             raise
    ...
    def _get_index_of_two_units_method1(self, unit1: Unit, unit2: Unit) -> int:
        assert unit1.tag in self._unit_index_dict, (
            ...
            f" as these contain unit data from the current frame. Do not try to save 'Units'"
            f" objects over several iterations."
        )
        assert unit2.tag in self._unit_index_dict, (
    ...
    @staticmethod
    def distance_math_hypot(po1: Tuple[float, float], po2: Tuple[float, float]):
        return math.hypot(po1[0] - po2[0], po1[1] - po2[1])

    @staticmethod
    def distance_math_hypot_squared(po1: Tuple[float, float], po2: Tuple[float, float]):
        return math.hypot(po1[0] - po2[0], po1[1] - po2[1]) ** 2
    ...
    def _distance_squared_unit_to_unit_method1(self, unit1: Unit, unit2: Unit) -> float:
        # If checked on units if they have the same tag,
        # return distance 0 as these are not in the 1 dimensional pdist array - would result in an error otherwise
        if unit1.tag == unit2.tag:
            return 0
