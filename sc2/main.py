...
async def _play_game_ai(client, player_id, ai, realtime, step_time_limit, game_time_limit):
    ...
    ai._prepare_step(game_state, proto_game_info)
        ...
        try:
            if realtime:
                ...
                if time_penalty_cooldown > 0:
                    ...
                    await ai._after_step()
        except ProtocolError as protocol_exc:
            if protocol_exc.is_game_over_error:
                if realtime:
                    return None
...
async def _setup_host_game(server, map_settings, players, realtime, random_seed=None):
    request = await server.create_game(map_settings, players, realtime, random_seed)
    if request.create_game.HasField("error"):
        err = f"Could not create game: {CREATE_GAME_ERROR(request.create_game.error)}"
        if request.create_game.HasField("error_details"):
            err += f": {request.create_game.error_details}"
        LOGGER.critical(err)
