...
from .data import ALLIANCE, ATTRIBUTE, CLOAK_STATE, RACE, warpgate_abilities, TARGET
...
class Unit:
        ...
        if self._weapons():
            for weapon in self._weapons():
                if weapon.damage_bonus:
                    bonus = weapon.damage_bonus[0]
                    return bonus.bonus, ATTRIBUTE(bonus.attribute).name
            return None
        return None
    ...
    def distance_to(self, pos: Union[Unit, Point2, Point3]) -> Union[int, float]:
        ...
        if isinstance(pos, Unit):
            return self._bot_object._distance_squared_unit_to_unit(self, pos) ** 0.5
        return self._bot_object.distance_math_hypot(self.position_tuple, pos)
    ...
    def in_ability_cast_range(
        ...
        """ Test if a unit is able to cast an ability on the target without checking ability cooldown
         (like stalker blink) or if ability is made available through research (like HT storm).
    ...
    def is_facing(self, other_unit: Unit, angle_error: float = 0.05) -> bool:
        """ Check if this unit is facing the target unit. If you make angle_error too small,
         there might be rounding errors. If you make angle_error too big, this function might return false positives.
    ...
    def is_detector(self) -> bool:
        ...
        return self.is_ready and (
            self.type_id() in IS_DETECTOR or self.type_id() == UNIT_PHOTONCANNON and self.is_powered
        )
    ...
    def passengers(self) -> Set[Unit]:
        """ Returns units inside a Bunker, CommandCenter, PlanetaryFortress, Medivac, Nydus, Overlord or WarpPrism. """
        return {Unit(unit, self._bot_object) for unit in self._proto.passengers}

    @property_mutable_cache
    def passengers_tags(self) -> Set[int]:
        """ Returns the tags of the units inside a Bunker, CommandCenter,
        PlanetaryFortress, Medivac, Nydus, Overlord or WarpPrism. """
