...
class Units(list):
    ...
    def in_attack_range_of(self, unit: Unit, bonus_distance: Union[int, float] = 0) -> Units:
        ...
        Caution: This may not work well for static structures (bunker, sieged tank, planetary fortress, photon cannon,
        spine and spore crawler) because it seems attack ranges differ for static / immovable units.
    ...
    def closer_than(self, distance: Union[int, float], position: Union[Unit, Point2, Point3]) -> Units:
        ...
        Example::
            ...
            if my_marine:
                ...
                # Contains all zerglings that are distance 3 or less away from the marine (does not include unit radius
                 in calculation)
    ...
    def in_distance_between(
        ...
        Example::
            ...
            if my_marine:
                ...
                # Contains all zerglings that are between distance 3 and 5 away from the marine (does not include unit
                 radius in calculation)
    ...
    def n_closest_to_distance(self, position: Union[Point2, np.ndarray], distance: Union[int, float], n: int) -> Units:
        ...
        For example if the dist is set to 5 and you want 3 units, from units with distance [3, 4, 5, 6, 7] to position,
    ...
    def filter(self, pred: callable) -> Units:
        ...
        Example::
            ...
            queens_with_energy_to_inject = self.units.filter(lambda unit: unit.type_id == UnitTypeId.QUEEN
            and unit.energy >= 25)

            orbitals_with_energy_to_mule = self.structures.filter(lambda structure: structure.type_id ==
            UnitTypeId.ORBITALCOMMAND and structure.energy >= 50)
    ...
    def tags_in(self, other: Union[Set[int], List[int], Dict[int, Any]]) -> Units:
        ...
        Example::
            ...
            # Do not use the following as it is slower because it first loops over all units to filter out if they are
             queens and loops over those again to check if their tags are in the list/set
    ...
    @property
    def idle(self) -> Units:
        """ Returns all units or structures that are doing nothing (unit standing still, structure doing nothing). """
